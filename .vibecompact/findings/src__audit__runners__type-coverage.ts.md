# src/audit/runners/type-coverage.ts

Single-lane finding (below the corroboration gate — one signal, weigh accordingly) · firing: deadcode · 6 lanes applicable · anchor `e147b1d50b7a`

### deadcode — 2 of 4 exported items unconsumed

| item | line | action |
|---|---|---|
| `expandSolutionTsconfig` | 61 | un-export — used inside this file |
| `dependenciesInstalled` | 103 | un-export — used inside this file |

Items marked *un-export* are live code — only their `export` keyword is unconsumed. Remove the keyword; deleting the symbol would break this file.

### If this finding is wrong or accepted

```
vibecheck wontfix|noise|justify "deadcode:src/audit/runners/type-coverage.ts" --reason "..."
```
